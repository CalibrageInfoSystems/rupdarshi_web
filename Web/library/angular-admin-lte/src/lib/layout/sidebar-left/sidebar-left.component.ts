...
export class SidebarLeftComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private filterMenuList: Array<any> = [];
  userInfo: any;
  ...
  ngOnInit() {
    this.subscriptions.push(this.layoutStore.sidebarLeftMenu.subscribe(value => {
      this.menu = value;
      var userActivityRights = this.userActivityRights;
      this.menu.forEach(item => {
        this.filterMenuList=[];
        for (var key in userActivityRights) {
          if (userActivityRights[key]) {
            if(item.children){
              for(var i=0;i<item.children.length;i++){
                if(key == item.children[i].ActivityRight){
                  if(item.label == "Security"){
                    this.filterMenuList.push(item);
                    if(this.filterMenuList.length == 2){
                      this.menuList.push(item);
                    }
                  }
                  if(item.label == "Masters"){
                    this.filterMenuList.push(item);
                    if(this.filterMenuList.length == 3){
                      this.menuList.push(item);
                    }
                  }
                }
            ...
            else if(key == item.ActivityRight) {
              this.menuList.push(item);
            }
          }
        }
