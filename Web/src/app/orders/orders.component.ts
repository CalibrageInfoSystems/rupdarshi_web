...
export class OrdersComponent implements OnInit {
  ...
  usersList:any[]=[];
  rowEdit:any;
  assignDeliveryAgentForm:FormGroup;
  ...
  constructor(private fb:FormBuilder, private spinner: NgxSpinnerService, private _dataService: DataService, private toastr: ToastrService) { 
    ...
    this.today=new Date();
    this.orderStatus=DataFactory.OrderStatus.Processed;
    this.orderSts=DataFactory.OrderStatus.Delivered;
    this.assignDeliveryAgentForm=fb.group({
  ...
  onSearchClick(){
    ...
    this._dataService.Post('Order/GetOrdersByStore',req)
    .subscribe((Data) => {
      if (Data.IsSuccess) {
  ...
  onAssignDeliveryAgent(){
    ...
    var req = {
      ...
      "DeliveryAgentId": this.user,
      "UpdatedByUserId": this.currentUser.Id,
      "UpdatedDate": new Date()
    }
    this._dataService.Post('Order/AssignOrdertoDeliveryAgent', req).subscribe((response) => {
      if (response.IsSuccess) {
        this.spinner.hide();
        this.assignDeliveryAgentForm.reset();
        this.onAssignDeliveryCancel();
        this.toastr.success(response.EndUserMessage);
        this.onSearchClick();
        $("#assignDeliveryAgent").modal("hide");
      }
      else {
        this.spinner.hide();
  ...
  getUserByroleAndStore() {
    var req={
      "RoleId": DataFactory.RoleIds.deliveryAgent,
      "StoreId": this.storeId
    }
    this._dataService.Post('Roles/GetUsersByRoleAndStore', req)
      .subscribe((Data) => {
        if (Data.IsSuccess) {
  ...
  assignDeliveryAgent(ord){
    this.user=null;
    this.getUserByroleAndStore()
    this.rowEdit=ord;
  }
  ...
  onAssignDeliveryCancel(){
    const { assignDeliveryAgentForm: { value: formValueSnap } } = this;
    this.assignDeliveryAgentForm.reset(formValueSnap); 
    // this.assignDeliveryAgentForm.reset();
  }
